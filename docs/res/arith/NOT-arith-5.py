...
powers_A = [11,12]  # which powers to show for group A
...
powers_B = [3]  # which powers to show for group B

# ----------------- Zoom Parameters -----------------
zoom_start = 0.57568359375  # start of zoom range [0, 1]
zoom_end = 0.576265    # end of zoom range [0, 1]
...
group_A_min_label_width_frac = 0.13  # Only show labels if segment width >= this fraction
...
group_B_min_label_width_frac = 0.06  # Only show labels if segment width >= this fraction
...
for line_idx, (power, y) in enumerate(zip(powers_B, y_positions_B)):
    ...
    for seg in visible_segments:
        # Only show label if segment is complete and wide enough in display coordinates
        if seg['display_width'] >= min_width:
            x_center = (seg['display_start'] + seg['display_end']) / 2.0
            ax.text(x_center, y - group_B_label_offset, seg['string'], 
                   ha='center', va='top', fontsize=group_B_fontsize, 
                   color=group_B_color, zorder=4)

# ----------------- Highlight Line Parameters -----------------
highlight_bounds = (0.575927734375,0.576171875) # interval in original coordinates where highlight appears
highlight_color = 'red'
highlight_linewidth = 6

# Calculate position between groups
if n_lines_A > 0 and n_lines_B > 0:
    highlight_y = (y_positions_A[-1] + y_positions_B[0]) / 2
elif n_lines_A > 0:
    highlight_y = y_positions_A[-1] - margin_between_groups / 2
elif n_lines_B > 0:
    highlight_y = y_positions_B[0] + margin_between_groups / 2
else:
    highlight_y = 0.5  # fallback

# Check if highlight interval overlaps with zoom range
highlight_start, highlight_end = highlight_bounds
if highlight_end > zoom_start and highlight_start < zoom_end:
    # Calculate visible portion of highlight
    vis_highlight_start = max(highlight_start, zoom_start)
    vis_highlight_end = min(highlight_end, zoom_end)
    
    # Map to display coordinates
    display_highlight_start = x0 + (vis_highlight_start - zoom_start) / (zoom_end - zoom_start) * x_len
    display_highlight_end = x0 + (vis_highlight_end - zoom_start) / (zoom_end - zoom_start) * x_len
    
    # Draw thick red highlight line
    ax.hlines(highlight_y, display_highlight_start, display_highlight_end, 
              colors=highlight_color, linewidth=highlight_linewidth, zorder=5)
