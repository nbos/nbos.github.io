...
# ----------------- Zoom Parameters -----------------
zoom_start = 0.0  # start of zoom range [0, 1]
...
group_A_min_label_width_frac = 0.08  # Only show labels if segment width >= this fraction
...
group_B_tick_half_height = 0.065 # half height of division markers
group_B_min_label_width_frac = 0.05  # Only show labels if segment width >= this fraction
...
# ----------------- Y positions with dynamic spacing -----------------
x0, x1 = x_range
x_len = x1 - x0
zoom_scale = 1.0 / (zoom_end - zoom_start)

# Define reduced spacing for when there are no labels between lines
reduced_line_spacing_A = 0.13  # smaller spacing for group A when no labels
reduced_line_spacing_B = 0.08  # smaller spacing for group B when no labels

def line_has_labels(alphabet, power, min_label_width_frac):
    """Check if a line will have any visible labels."""
    strings_probs = generate_alphabet_powers(alphabet, power)
    ...
    visible_segments, _ = get_visible_segments(cdf_edges, strings_probs, zoom_start, zoom_end)
    
    min_width = min_label_width_frac * x_len
    for seg in visible_segments:
        if seg['is_complete'] and seg['display_width'] >= min_width:
            return True
    return False

# Check which lines will have labels
lines_A_have_labels = [line_has_labels(alphabet_A, power, group_A_min_label_width_frac) for power in powers_A]
lines_B_have_labels = [line_has_labels(alphabet_B, power, group_B_min_label_width_frac) for power in powers_B]

# Group A lines: start from top and space downward
y_top = 1.0 - margin_top
y_positions_A = []
if n_lines_A > 0:
    y_positions_A.append(y_top)
    
    for i in range(1, n_lines_A):
        # Use reduced spacing if the line below has no labels
        spacing = reduced_line_spacing_A if not lines_A_have_labels[i] else line_spacing_A
        y_positions_A.append(y_positions_A[-1] - spacing)
...
y_positions_B = []
if n_lines_B > 0:
    y_positions_B.append(y_start_B)
    
    for i in range(1, n_lines_B):
        # Use reduced spacing if the line above has no labels
        spacing = reduced_line_spacing_B if not lines_B_have_labels[i-1] else line_spacing_B
        y_positions_B.append(y_positions_B[-1] - spacing)
